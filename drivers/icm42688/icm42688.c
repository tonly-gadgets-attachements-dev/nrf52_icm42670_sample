...
static int icm42688_set_accel_odr(const struct device *dev, uint16_t rate)
{
	...
	if (rate > 16000) {
		...
		temp = BIT_ACCEL_ODR_12_5;
	} else if (rate > 3) {
		temp = BIT_ACCEL_ODR_6_25;
	} else if (rate > 1) {
		temp = BIT_ACCEL_ODR_3_12;
	} else {
		temp = BIT_ACCEL_ODR_1_5625;
	}

	return icm42688_spi_update_register(&cfg->spi, REG_ACCEL_CONFIG0, (uint8_t)MASK_ACCEL_ODR,
					    temp);
}
...
static int icm42688_sensor_init(const struct device *dev)
{
	...
	res = icm42688_spi_single_write(&cfg->spi, REG_DEVICE_CONFIG, BIT_SOFT_RESET);
